...
public class TaskController {
    ...
    @PostMapping
    public ResponseEntity<TaskResponse> create(@RequestBody TaskCreate dto) {
        TaskResponse response = taskService.create(dto);
        return new ResponseEntity<>(response, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<TaskResponse> update(@PathVariable Long id, @RequestBody TaskUpdate dto) {
