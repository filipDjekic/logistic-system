...
export default function WarehouseDetailsPage() {
  ...
  const inventoryQuery = useInventory({ search: inventorySearch, warehouseId: id ?? 'ALL', productId: 'ALL', status: 'ALL', page: inventoryPage.page, size: inventoryPage.size, sort: 'product.name,asc' }, id != null && activeTab === 'inventory');
  const movementQuery = useStockMovements({ search: movementSearch, movementType, warehouseId: id ?? 'ALL', productId: 'ALL', transportOrderId: 'ALL', fromDate: '', toDate: '', page: movementPage.page, size: movementPage.size, sort: 'createdAt,desc' }, id != null && activeTab === 'stockMovements');
  ...
  return <EntityDetailsLayout overline="Storage" title={warehouse.name} description={`${warehouse.address}, ${warehouse.cityName ?? warehouse.city}`} tabs={tabs} activeTab={activeTab} onTabChange={(value) => setActiveTab(value as WarehouseDetailsTab)} actionItems={[
    ...
    {activeTab === 'stockMovements' ? <RelatedDataSection title="Warehouse stock movements" description="Stock movement history for this warehouse." loading={movementQuery.isLoading} error={movementQuery.isError} onRetry={() => void movementQuery.refetch()} empty={!movementQuery.isLoading && (movementQuery.data?.content.length ?? 0) === 0} emptyTitle="No stock movements" emptyDescription="No stock movements have been recorded for this warehouse."><Stack spacing={2}><Grid container spacing={2}><Grid size={{ xs: 12, md: 8 }}><TextField fullWidth size="small" label="Search movements" value={movementSearch} onChange={(event) => setMovementSearch(event.target.value)} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth select size="small" label="Movement type" value={movementType} onChange={(event) => setMovementType(event.target.value as StockMovementFiltersState['movementType'])}><MenuItem value="ALL">All movement types</MenuItem><MenuItem value="INBOUND">Inbound</MenuItem><MenuItem value="OUTBOUND">Outbound</MenuItem><MenuItem value="TRANSFER_IN">Transfer in</MenuItem><MenuItem value="TRANSFER_OUT">Transfer out</MenuItem><MenuItem value="ADJUSTMENT">Adjustment</MenuItem><MenuItem value="WRITE_OFF">Write off</MenuItem><MenuItem value="RETURN_IN">Return in</MenuItem><MenuItem value="RETURN_OUT">Return out</MenuItem></TextField></Grid></Grid><StockMovementsTable rows={movementQuery.data?.content ?? []} loading={movementQuery.isLoading} error={movementQuery.isError} onRetry={() => void movementQuery.refetch()} pagination={movementPage.pagination(movementQuery.data, movementQuery.isFetching)} /></Stack></RelatedDataSection> : null}
