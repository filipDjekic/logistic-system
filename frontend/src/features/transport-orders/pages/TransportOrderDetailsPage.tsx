...
} from "../components/details/transportOrderDetailsUtils";
...
export default function TransportOrderDetailsPage() {
  ...
  const [selectedItem, setSelectedItem] =
    useState<TransportOrderItemResponse | null>(null);
  const [orderDialogOpen, setOrderDialogOpen] = useState(false);
  const [activeTab, setActiveTab] =
    useState<TransportOrderDetailsTab>("overview");
  const [transitionTarget, setTransitionTarget] =
    useState<TransportOrderStatus | null>(null);
  ...
  if (transportOrderQuery.isLoading) {
    return (
      <EntityDetailsLayout
        ...
        loading
        loadingText="Loading transport order details..."
        actionItems={[
          {
            label: "Back to list",
  ...
  const tabs: { value: string; label: ReactNode; disabled?: boolean }[] = [
    ...
    { value: "items", label: "Transport items", disabled: !canReadItems },
    {
      value: "relatedStockMovements",
      label: "Stock movements",
      disabled: !canReadItems,
    },
    { value: "tasks", label: "Related tasks", disabled: !canReadItems },
    ...buildOperationalTabs({ entityType: "TRANSPORT_ORDER", entityName: "TRANSPORT_ORDER", entityId: transportOrder.id, allowCreateAttachments: canManageOrder, allowCreateComments: canManageOrder, canViewAudit: canViewHistory }),
  ];
    ...
    <EntityDetailsLayout
      ...
      actionItems={[
        ...(isEditableOrder
          ...
          : []),
        { label: "Back to list", onClick: () => navigate("/transport-orders") },
      ]}
    >
      {activeTab === "overview" ? (
            ...
            <TransportOrderItemsTable
              ...
              deletingItemId={
                deleteItemMutation.isPending
                  ? (deleteItemMutation.variables ?? null)
                  : null
              }
                ...
                <Grid container spacing={2}>
                    ...
                    <Controller
                      ...
                      render={({ field, fieldState }) => {
                        const selectedProduct =
                          productsById[Number(field.value)];
                              ...
                              field.value
                                ? {
                                    ...
                                    subtitle: selectedProduct
                                      ? `${selectedProduct.unit} · ${formatWeight(selectedProduct.weight)}`
                                      : null,
                                  }
                                : null
                            }
                  ...
                  <Grid size={{ xs: 12 }}>
                    <Stack spacing={1.5}>
                      ...
                      <FormActions
                        cancelLabel={
                          selectedItem ? "Cancel edit" : "Clear item form"
                        }
                        submitLabel={
                          selectedItem ? "Save item changes" : "Add item"
                        }
                        submittingLabel={
                          selectedItem ? "Saving item..." : "Adding item..."
                        }
      ...
      {activeTab === "relatedStockMovements" ? (
        <RelatedDataSection
          ...
          empty={
            !relatedStockMovementsQuery.isLoading &&
            !relatedStockMovementsQuery.isError &&
            (relatedStockMovementsQuery.data?.content ?? []).length === 0
          }
      ...
      {activeTab === "tasks" ? (
        <RelatedDataSection
          ...
          empty={
            ...
            (relatedTasksQuery.data?.content ?? []).length === 0
          }
      ...
      <OperationalDetailsTabPanels
        ...
        allowCreateAttachments={canManageOrder}
        allowCreateComments={canManageOrder}
        canViewAudit={canViewHistory}
        auditUnavailableTitle="Audit unavailable"
        auditUnavailableDescription="Your role cannot view transport order audit data."
      />
