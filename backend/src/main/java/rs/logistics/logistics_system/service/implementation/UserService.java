...
public class UserService implements UserServiceDefinition {
    ...
    public UserResponse update(Long id, UserUpdate dto) {
        ...
        if (Boolean.TRUE.equals(dto.getEnabled())
                ...
                && Boolean.FALSE.equals(user.getEmployee().getActive())) {
    ...
    private Employee toEmployee(User user, Company company, UserEmployeeCreate employeeData) {
        ...
        Warehouse primaryWarehouse = employeeData.getPrimaryWarehouseId() == null
                ? null
                : warehouseRepository.findById(employeeData.getPrimaryWarehouseId())
        ...
        employee.setEmailGenerationSource("MANUAL_USER_CREATE");
        return employee;
    }
    ...
    private void syncLinkedEmployee(User user, UserEmployeeUpdate employeeDto) {
        ...
        if (Boolean.TRUE.equals(employeeDto.getApplyGeneratedEmailSuggestion())) {
        ...
        } else if (oldEmail != null && !oldEmail.equalsIgnoreCase(user.getEmail())) {
            employee.setAutoGeneratedEmail(false);
            employee.setEmailManuallyOverridden(true);
