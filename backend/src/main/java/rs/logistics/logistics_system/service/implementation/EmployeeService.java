...
public class EmployeeService implements EmployeeServiceDefinition {
    ...
    public EmployeeResponse create(EmployeeCreate dto) {
        ...
        employee.setEmailGenerationSource("MANUAL_CREATE");
    ...
    public EmployeeResponse createWithUser(EmployeeWithUserCreate dto) {
        ...
        String generatedEmail = employeeEmailGenerator.generateUnique(dto.getFirstName(), dto.getLastName(), company, dto.getPosition(), country);
        ...
        employee.setEmailGenerationSource(EmployeeEmailGenerator.SOURCE_EMPLOYEE_CREATE_WITH_USER);
    ...
    public EmployeeResponse update(Long id, EmployeeUpdate dto) {
        ...
        boolean applyGeneratedEmail = Boolean.TRUE.equals(dto.getApplyGeneratedEmailSuggestion());
        String requestedEmail = applyGeneratedEmail
                ? employeeEmailGenerator.generateUniqueExcluding(
                dto.getFirstName(),
                dto.getLastName(),
                employee.getCompany(),
                employee.getPosition(),
                resolveEmailCountryForUpdate(employee, dto.getCountryId()),
                employee.getUser() != null ? employee.getUser().getId() : null,
                employee.getId()
        )
                : normalizeEmail(dto.getEmail());

        validateUniqueEmailForUpdate(employee.getId(), requestedEmail, employeeCompanyId);
        validateUniqueUserEmailForUpdate(employee.getUser() != null ? employee.getUser().getId() : null, requestedEmail);

        dto.setEmail(requestedEmail);
        EmployeeMapper.updateEntity(dto, employee, employee.getUser(), null);
        if (applyGeneratedEmail) {
            employee.setAutoGeneratedEmail(true);
            employee.setEmailManuallyOverridden(false);
            employee.setEmailGenerationSource(EmployeeEmailGenerator.SOURCE_EMPLOYEE_UPDATE_SUGGESTION);
        } else if (!oldEmail.equalsIgnoreCase(requestedEmail)) {
            employee.setAutoGeneratedEmail(false);
            employee.setEmailManuallyOverridden(true);
            employee.setEmailGenerationSource("MANUAL_UPDATE");
        }
        if (employee.getUser() != null) {
            employee.getUser().setEmail(employee.getEmail());
        }
        applyEmployeeLocationDefaults(employee, dto.getCountryId(), dto.getCityId(),
                employee.getPrimaryWarehouse() != null ? employee.getPrimaryWarehouse().getId() : null, employee.getCompany());
        applyRequestedEmployeeTimezone(employee, dto.getTimezoneId());
        domainScopeValidator.ensureEmployeeLocationConsistency(employee);
        domainScopeValidator.ensureEmployeeCanBelongToPrimaryWarehouse(employee);
    ...
    public EmployeeResponse reactivateEmployee(Long id) {
        ...
        auditFacade.recordStatusChange("EMPLOYEE", id, "active", oldActive, saved.getActive());
        auditFacade.log(
                "REACTIVATE",
                "EMPLOYEE",
                id,
                saved.getEmail(),
                "EMPLOYEE was reactivated (ID: " + id + ")"
        );
    ...
    @Override @Transactional
    public EmployeeResponse changePrimaryWarehouse(Long id, EmployeePrimaryWarehouseUpdate dto) {
        Employee employee = getEmployeeOrThrow(id);
        ...
        boolean conflictingTask = employee.getTasks().stream().anyMatch(t -> t.getStockMovement() != null
                && t.getStockMovement().getWarehouse() != null
                && java.util.Objects.equals(t.getStockMovement().getWarehouse().getId(), oldId)
                && !Set.of(rs.logistics.logistics_system.enums.TaskStatus.COMPLETED, rs.logistics.logistics_system.enums.TaskStatus.CANCELLED).contains(t.getStatus()));
        if (conflictingShift || conflictingTask) throw new BadRequestException("Primary warehouse cannot be changed while active operations use the current warehouse.");
        Warehouse warehouse = dto.getWarehouseId() == null ? null : warehouseRepository.findByIdAndCompany_Id(dto.getWarehouseId(), requireEmployeeCompanyId(employee))
                .orElseThrow(() -> new ResourceNotFoundException("Warehouse not found"));
        if (warehouse != null && !Boolean.TRUE.equals(warehouse.getActive())) throw new BadRequestException("Primary warehouse must be active.");
    ...
    @Override @Transactional
    public EmployeeResponse linkUser(Long id, EmployeeUserLinkUpdate dto) {
        Employee employee = getEmployeeOrThrow(id);
        User user = resolveOptionalUser(dto.getUserId());
        validateUserAssignmentForUpdate(employee, user.getId());
        validateUserBelongsToCompany(user, requireEmployeeCompanyId(employee));
        rolePositionPolicy.validatePositionMatchesRole(employee.getPosition(), user.getRole());
        Long old = employee.getUser() != null ? employee.getUser().getId() : null;
        employee.setUser(user);
        Employee saved = _employeeRepository.save(employee);
        auditFacade.recordFieldChange("EMPLOYEE", id, "user_id", old, user.getId());
        return EmployeeMapper.toResponse(saved);
    }

    @Override @Transactional
    public EmployeeResponse unlinkUser(Long id) {
        Employee employee = getEmployeeOrThrow(id);
        if (employee.getUser() == null) throw new BadRequestException("Employee has no linked user.");
        Long userId = employee.getUser().getId();
        if (Boolean.TRUE.equals(employee.getUser().getEnabled())) userService.disableUser(userId);
        employee.setUser(null);
        Employee saved = _employeeRepository.save(employee);
        auditFacade.recordFieldChange("EMPLOYEE", id, "user_id", userId, null);
        return EmployeeMapper.toResponse(saved);
    }
    ...
    private void validateUniqueUserEmailForUpdate(Long currentUserId, String email) {
        if (currentUserId == null) {
            validateUniqueUserEmail(email);
            return;
        }
        if (_userRepository.existsByEmailIgnoreCaseAndIdNot(normalizeEmail(email), currentUserId)) {
            throw new BadRequestException("User with this email already exists");
        }
    }

    private Country resolveEmailCountryForUpdate(Employee employee, Long requestedCountryId) {
        if (requestedCountryId != null) {
            return countryRepository.findById(requestedCountryId)
                    .orElseThrow(() -> new ResourceNotFoundException("Country not found"));
        }
        if (employee.getCountry() != null) {
            return employee.getCountry();
        }
        return employee.getCompany() != null ? employee.getCompany().getCountry() : null;
    }
